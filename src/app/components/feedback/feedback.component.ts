...
export class FeedbackComponent implements OnInit {
  ...
  async hasAlreadyRated() {
    const res = await this.userService.hasUserAlreadyRated(
      this.feedBackEvent.creatorId,
      this.feedBackEvent.eventId
    );
  ...
  async saveFeedback() {
    ...
    await this.userService
      .addFeedback(newFeedback, this.feedBackEvent.creatorId)
      .then(() => {
