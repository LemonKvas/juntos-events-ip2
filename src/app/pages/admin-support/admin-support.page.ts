...
export class AdminSupportPage {
   ...
   */
  async getTickets() {
    (await this.supportService.getAllDoneTickets()).subscribe(
      (tickets) => (this.doneTickets = tickets)
    );
    (await this.supportService.getAllInProcessTickets()).subscribe(
      (tickets) => (this.inProcessTickets = tickets)
    );
    (await this.supportService.getAllOpenTickets()).subscribe(
      (tickets) => (this.openTickets = tickets)
    );
  }
  ...
  async openEvent(eventId) {
    await this.eventService.getEventById(eventId).then(async (ev) => {
  ...
  async openProfile(userId) {
    await this.userDataService.getUserById(userId).then(async (userData) => {
      const actionSheet = await this.actionSheetController.create({
